#!/usr/bin/env bash
#
# git-surgeon installation script
# Usage: curl -fsSL https://raw.githubusercontent.com/raine/git-surgeon/main/scripts/install.sh | bash
#
# Environment variables:
#   GIT_SURGEON_VERSION      - Pin a specific version (e.g., v0.1.0)
#   GIT_SURGEON_INSTALL_DIR  - Override install directory (default: /usr/local/bin or ~/.local/bin)
#
# Examples:
#   GIT_SURGEON_VERSION=v0.1.0 bash install.sh
#   GIT_SURGEON_INSTALL_DIR=/opt/bin bash install.sh
#
...
detect_platform() {
	...
	*)
		...
		echo ""
		echo "git-surgeon supports macOS and Linux."
		echo "For other platforms, try building from source with Cargo:"
		echo "  cargo install git-surgeon"
		echo ""
		exit 1
		;;
	esac
	...
	*)
		...
		echo ""
		echo "git-surgeon prebuilt binaries are available for amd64 and arm64."
...
install_from_release() {
	log_info "Installing git-surgeon from GitHub releases..."
	...
	local version="${GIT_SURGEON_VERSION:-}"

	if [ -z "$version" ]; then
		log_info "Fetching latest release..."
		local latest_url="https://api.github.com/repos/raine/git-surgeon/releases/latest"
		local release_json
		...
		if [ -z "$version" ]; then
			...
			echo "  GIT_SURGEON_VERSION=v0.1.0 bash install.sh"
			echo ""
			exit 1
		fi
	fi
	...
	# Download URL
	local archive_name="git-surgeon-${platform}.tar.gz"
	local download_url="https://github.com/raine/git-surgeon/releases/download/${version}/${archive_name}"
	...
	local checksum_url="https://github.com/raine/git-surgeon/releases/download/${version}/${checksum_file}"
	...
	local install_dir="${GIT_SURGEON_INSTALL_DIR:-}"
	...
	if [ -f "$install_dir/git-surgeon" ]; then
		local existing_version
		existing_version=$("$install_dir/git-surgeon" --version 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
	...
	local tmp_binary="$install_dir/git-surgeon.tmp.$$"

	if [[ -w "$install_dir" ]]; then
		cp git-surgeon "$tmp_binary"
		chmod +x "$tmp_binary"
		mv -f "$tmp_binary" "$install_dir/git-surgeon"
	else
		if ! sudo cp git-surgeon "$tmp_binary"; then
			log_error "Failed to install to $install_dir (sudo required)"
			exit 1
		fi
		sudo chmod +x "$tmp_binary"
		sudo mv -f "$tmp_binary" "$install_dir/git-surgeon"
	fi

	# Remove macOS quarantine attribute if present
	if [[ "$(uname -s)" == "Darwin" ]] && command -v xattr &>/dev/null; then
		xattr -d com.apple.quarantine "$install_dir/git-surgeon" 2>/dev/null || true
	fi

	log_success "git-surgeon installed to $install_dir/git-surgeon"
...
verify_installation() {
	...
	if [ ! -x "$install_dir/git-surgeon" ]; then
		log_error "git-surgeon binary not found or not executable at $install_dir/git-surgeon"
		exit 1
	fi

	# Test the binary works
	if ! "$install_dir/git-surgeon" --version &>/dev/null; then
		log_error "git-surgeon binary exists but failed to run"
		exit 1
	fi

	log_success "git-surgeon is installed and ready!"
	echo ""
	"$install_dir/git-surgeon" --version
	echo ""
}
