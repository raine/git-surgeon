...
def test_amend_head(git_agent_exe, repo):
...
def test_amend_earlier_commit(git_agent_exe, repo):
    ...
    # Stage a change to amend into the first file's commit
...
def test_amend_preserves_unstaged_changes(git_agent_exe, repo):
    ...
    # Create another commit so we have something to amend into
...
def test_amend_root_commit(git_agent_exe, repo):
    ...
    result = run_git_agent(git_agent_exe, repo, "amend", root_sha)
    assert result.returncode == 0
